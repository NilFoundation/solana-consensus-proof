    ...
    \item $sB ?=? R + kA$:
        \begin{enumerate}
            ...
            \item One addition is used for $S + (-R)$. The coordinates of $R$ and $T = S + (-R)$ are placed on the last row of fixed-base scalar multiplication circuit.
                ...
                \begin{center}
                    $w_{3, j + 84}\cdot (1 + d w_{6, j + 84} \cdot (-w_{1, j + 84}) \cdot w_{7, j + 84} \cdot w_{2, j + 84}) = w_{6, j + 84} \cdot w_{2, j + 84} + (-w_{1, j + 84}) \cdot w_{7, j + 84}$ \\
                    $w_{4, j + 84} \cdot (1 - d w_{6, j + 84} \cdot (-w_{1, j + 84}) \cdot w_{7, j + 84} \cdot w_{2, j + 84}) = w_{6, j + 84} \cdot (-w_{1, j + 84}) + w_{2, j + 84} \cdot w_{7, j + 84}$ \\
                    $(- w_{1, j + 84})^2 + w_{2, j + 84}^2 = 1 - d \cdot w_{1, j + 84}^2 \cdot w_{2, j + 84}^2$
                \end{center}
            \item Variable-base scalar multiplication circuit for $T = k \cdot A$, where cells $w_{1, j + 254}, w_{2, j + 254}$ are copy constrained with $w_{3, j + 84}, w _{5, j + 84}$ from the fixed-base scalar multiplication circuit.
        \end{enumerate}
\end{enumerate}
